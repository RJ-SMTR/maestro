...
@solid(
    config_schema={"table_id": str, "query_table": str},
    required_resource_keys={"basedosdados_config", "bd_client", "schedule_run_date"},
)
def create_registros_1_dia(context):
    ...
    query = f"""
    create table {project}.{dataset_id}.{table_id} as
    SELECT * 
    ...
    if tb.table_exists("prod"):
        tb.delete("prod")

    context.resources.bd_client.query(query).result()
...
def get_daily_data(context, run_date):
...
def projeto_multa_automatica_multas_aplicaveis_onibus():
    upload_table(get_daily_data(create_registros_1_dia()))
